...
`resources/tmdbhelper/lib/items/database/mappings/__init__.py` (**correction, post-implementation**: originally planned as a persisting flat `mappings.py` file, but Python cannot resolve a `.py` module and a same-named sibling package directory at once — the `.py` file always wins and the directory becomes entirely unreachable. The aggregate had to move into the new package as its `__init__.py` instead, which resolves identically for `from tmdbhelper.lib.items.database.mappings import ItemMapper` — the external import path is unaffected. See the "Package structure note" below for the same correction applied to the no-`__init__.py` claim.) shrinks to:
...
**Correction, post-implementation:** this section originally claimed no `__init__.py` was needed in the new `mappings/` directory, following the `baseitem_factories/concrete_classes/`-style implicit-namespace-package convention used elsewhere in this codebase. That assumption was wrong for this specific case: `mappings/` needed an `__init__.py` after all, because the old `mappings.py` had to keep existing as *something* importable at `tmdbhelper.lib.items.database.mappings` (for `ItemMapper` to stay reachable at its unchanged path) — and Python can't have a `mappings.py` file and a same-named sibling `mappings/` directory coexist. `mappings/__init__.py` is that something; it now holds the seven-mixin aggregate `ItemMapperMethods` plus `BlankNoneDict` and `ItemMapper`, exactly as this section originally described, just at a different physical filename. Every other mixin file (`support.py`, `general.py`, `art.py`, `genre.py`, `credits.py`, `translations.py`, `movie.py`, `tv.py`) remains a plain sibling module with no `__init__.py` of its own — only the aggregate file needed one. (The `api/openrouter/` inconsistency noted in the original text is unrelated and still stands as its own separate, unfixed inconsistency.)
